...
from sqlalchemy import Column, String, Integer, Boolean, TIMESTAMP, DECIMAL, TEXT, JSON, Index, Float, Enum
...
class PriceSnapshot(Base):
    ...
    def _safe_float(self, value):
        """SQLAlchemy Column을 안전하게 float로 변환"""
        if value is None:
            return None
        try:
            return float(value)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "price": self._safe_float(self.price),
            "volume_24h": self._safe_float(self.volume_24h),
            "price_change_24h": self._safe_float(self.price_change_24h),
...
class KimchiPremium(Base):
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "coingecko_id": self.coingecko_id,
            "symbol": self.symbol,
            "upbit_price": self._safe_float(self.upbit_price),
            "bithumb_price": self._safe_float(self.bithumb_price),
            "korean_avg_price": self._safe_float(self.korean_avg_price),
            "global_avg_price": self._safe_float(self.global_avg_price),
            "global_avg_price_krw": self._safe_float(self.global_avg_price_krw),
            "usd_krw_rate": self._safe_float(self.usd_krw_rate),
            "kimchi_premium": self._safe_float(self.kimchi_premium),
...
class ExchangeRate(Base):
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "rate": self._safe_float(self.rate),
            "source": self.source,
            "updated_at": self.updated_at.isoformat() if self.updated_at is not None else None
        }
...
class LiquidationEvent(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    exchange = Column(Enum(ExchangeEnum), nullable=False, comment="거래소")
    symbol = Column(String(20), nullable=False, comment="심볼 (예: BTCUSDT)")
    timestamp = Column(TIMESTAMP, nullable=False, comment="청산 시각")
    
    # 청산 정보
    side = Column(Enum(PositionSideEnum), nullable=False, comment="포지션 사이드")
    price = Column(Float, nullable=False, comment="청산 가격")
    quantity = Column(Float, nullable=False, comment="청산 수량")
    value_usd = Column(Float, nullable=False, comment="청산 가치 (USD)")
    
    # 추가 정보
    order_id = Column(String(100), comment="주문 ID")
    leverage = Column(Float, comment="레버리지")
    ...
    # 인덱스 정의
    __table_args__ = (
        Index('idx_liquidation_symbol_time', 'symbol', 'timestamp'),
        Index('idx_liquidation_time', 'timestamp'),
        Index('idx_liquidation_exchange', 'exchange'),
        Index('idx_liquidation_value', 'value_usd'),
    )
    
    def __repr__(self):
        return f"<LiquidationEvent(symbol={self.symbol}, side={self.side.value}, value=${self.value_usd:.2f})>"
...
class LiquidationSummaryDaily(Base):
    """일별 청산 요약 데이터 (성능 최적화용)"""
    __tablename__ = "liquidation_summaries_daily"
    ...
    # 집계 데이터
    total_liquidation_usd = Column(Float, default=0, comment="총 청산액 (USD)")
    long_liquidation_usd = Column(Float, default=0, comment="롱 청산액 (USD)")
    short_liquidation_usd = Column(Float, default=0, comment="숏 청산액 (USD)")
    
    total_events = Column(Integer, default=0, comment="총 청산 건수")
    long_events = Column(Integer, default=0, comment="롱 청산 건수")
    short_events = Column(Integer, default=0, comment="숏 청산 건수")
    ...
    # 인덱스 정의
    __table_args__ = (
        Index('idx_liquidation_summary_symbol_date', 'symbol', 'date'),
        Index('idx_liquidation_summary_date', 'date'),
        Index('idx_liquidation_summary_updated', 'last_updated'),
    )
    
    def __repr__(self):
        return f"<LiquidationSummaryDaily(symbol={self.symbol}, date={self.date}, total_usd=${self.total_liquidation_usd:.2f})>"
...
def get_model_by_name(model_name: str):
    ...
    models = {
        ...
        "liquidation_summaries_daily": LiquidationSummaryDaily
    }
