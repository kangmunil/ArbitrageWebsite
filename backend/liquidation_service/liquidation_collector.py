...
class LiquidationDataCollector:
    ...
    async def start_collection(self):
        ...
        logger.info("LiquidationDataCollector.start_collection() called")
        if self.is_running:
        ...
        # 테스트 데이터 생성
        await self.generate_test_liquidation_data()
        ...
        logger.info(f"Starting {len(tasks)} liquidation collection tasks...")
        try:
    ...
    async def collect_bybit_liquidations(self):
        """바이비트 청산 데이터 수집."""
        logger.info("바이비트: 실제 WebSocket 연결 + 백업 시뮬레이션 데이터 사용")
        ...
        async def bybit_simulation_fallback():
            while self.is_running:
                try:
                    ...
                    logger.info(f"바이비트 시뮬레이션 청산: {liquidation['side']} {liquidation['quantity']:.2f} BTC")
        ...
        while self.is_running:
            try:
                ...
                async with websockets.connect(uri, ping_timeout=20, ping_interval=20) as websocket:
                    ...
                    logger.info("바이비트 청산 데이터 연결 성공! 구독 메시지 전송됨")
                    ...
                    async for message in websocket:
                        ...
                        if message_count % 5 == 0:  # 5개마다 로그
                            logger.info(f"바이비트 실제 메시지 {message_count}개 수신됨: {str(data)[:100]}...")
                        await self.process_bybit_liquidation(data)
                        
            except Exception as e:
                logger.error(f"바이비트 WebSocket 연결 오류: {e} - 시뮬레이션 데이터로 백업 중")
                await asyncio.sleep(10)  # 연결 재시도 전 대기
    
    async def collect_okx_liquidations(self):
        """OKX 청산 데이터 수집."""
        logger.info("OKX: WebSocket API 인증 필요로 인해 시뮬레이션 데이터 사용")
        ...
        while self.is_running:
            try:
                ...
                logger.info(f"OKX 시뮬레이션 청산: {liquidation['side']} {liquidation['quantity']:.2f} BTC")
                await self.store_liquidation_data(liquidation)
    ...
    async def collect_bitmex_liquidations(self):
        """BitMEX 청산 데이터 수집."""
        logger.info("BitMEX: API 인증 필요로 인해 시뮬레이션 데이터 사용")
        ...
        while self.is_running:
            try:
                ...
                logger.info(f"BitMEX 시뮬레이션 청산: {liquidation['side']} {liquidation['quantity']:.2f} BTC")
                await self.store_liquidation_data(liquidation)
    ...
    async def collect_bitget_liquidations(self):
        """Bitget 청산 데이터 수집."""
        logger.info("Bitget: WebSocket 연결 불안정으로 인해 시뮬레이션 데이터 사용")
        ...
        while self.is_running:
            try:
                ...
                logger.info(f"Bitget 시뮬레이션 청산: {liquidation['side']} {liquidation['quantity']:.2f} BTC")
                await self.store_liquidation_data(liquidation)
    ...
    async def collect_hyperliquid_liquidations(self):
        ...
        logger.info("Hyperliquid: DEX API 복잡성으로 인해 시뮬레이션 데이터 사용")
        ...
        while self.is_running:
            try:
                ...
                logger.info(f"Hyperliquid 시뮬레이션 청산: {liquidation['side']} {liquidation['quantity']:.2f} BTC")
                await self.store_liquidation_data(liquidation)
