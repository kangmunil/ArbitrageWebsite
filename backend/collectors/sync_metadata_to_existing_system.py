...
Cryptocurrency: Optional[Any] = None

try:
    from core.database import SessionLocal, engine
    import importlib
    models_module = importlib.import_module('core.models')
    Cryptocurrency = getattr(models_module, 'Cryptocurrency')
...
class MetadataSync:
    ...
    def check_existing_system(self):
        ...
        try:
            # 데이터베이스 연결 테스트
            if engine is None:
                return False, "데이터베이스 엔진이 초기화되지 않음"
    ...
    def check_target_data(self):
        ...
        try:
            if SessionLocal is None:
                raise Exception("SessionLocal이 초기화되지 않음")
            db = SessionLocal()
            try:
                if Cryptocurrency is None:
                    raise Exception("Cryptocurrency 모델이 초기화되지 않음")
